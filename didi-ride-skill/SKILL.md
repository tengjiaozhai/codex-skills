...
metadata:
  { "codex": { "emoji": "🚕", "always": true, "requires": { "bins": ["mcporter"], "env": ["DIDI_MCP_KEY"] }, "primaryEnv": "DIDI_MCP_KEY", "install": [{ "id": "node", "kind": "node", "package": "mcporter", "bins": ["mcporter"], "label": "Install mcporter (node)" }] } }
---
...
> Codex 可直接展示 Markdown 图片；如果客户端不显示图片，则直接给出官网链接。
...
**方式二：写入 Shell 配置（Codex 推荐）**

将环境变量写入你的 shell 启动文件，例如 `~/.zshrc`：
...
2. **检查 Key**：执行 `printenv DIDI_MCP_KEY`，若输出为空，按 `### 3.9 MCP KEY 与配置` 流程引导。Key 缺失时 mcporter 的报错信息具有误导性，不要尝试绕过。
   - ⚠️ **若 `DIDI_MCP_KEY` 已存在但 mcporter 仍报 `Missing KEY parameter`**：**不是 Key 失效**，**禁止向用户索要 Key**。排查步骤见 `references/error_handling.md` 中的「mcporter Missing KEY parameter」章节。
...
7. 结果收尾：默认提示用户可发送「查询订单」了解状态；只有用户**明确要求**“5 分钟后帮我查一下”或“自动提醒我”时，才使用 Codex 的自动化能力创建后续回查。
...
- 仅在用户**明确提出定时需求**时，使用 Codex 的自动化能力创建一次性任务；不要默认创建定时任务。
- 自动化任务内容必须包含完整起终点（带城市前缀）和车型，因为定时执行时不能依赖当前对话上下文。
- 起点或终点缺失时先从 `assets/PREFERENCE.md` 推断并向用户确认，推断不了则必须先询问用户，**不要自行推断未知信息**。
- 到点后的执行流程仍然是：地址解析 → 价格预估（获取最新 traceId）→ 创建订单。
- 如果当前环境不支持自动化，就明确告诉用户本次无法定时执行，请用户到时再来触发。
...
Codex 默认**不自动**创建回查任务，避免在用户未授权的情况下安排额外自动化。

- 发单成功后，默认提示用户发送「查询订单」了解最新状态。
- 只有当用户明确要求“5 分钟后帮我查一下”或“自动提醒我司机状态”时，才创建 Codex 自动化。
- 如果当前环境不支持自动化，就明确告诉用户无法自动回查，并提醒用户稍后手动发送「查询订单」。

### 3.9 MCP KEY 与配置

> **术语说明**：滴滴平台称此凭证为「MCP KEY」；在 Codex 中，直接使用环境变量 `DIDI_MCP_KEY`。

⚠️ Key 来源：建议写入 shell 启动文件（如 `~/.zshrc`），让每次新 shell / 新会话都能读取到 `DIDI_MCP_KEY`。
```bash
# ⚠️ 可以通过以下命令判断当前 DIDI_MCP_KEY 是否配置。
printenv DIDI_MCP_KEY
```

#### 3.9.1 用户在对话中提供 Key 时（必须持久化）

当用户回复了 Key（如"我的 Key 是 xxxxxx"），优先将其写入 shell 启动文件持久化；仅 `export` 到当前 shell 不够稳妥。

```bash
# ⚠️ 将 YOUR_KEY 替换为用户提供的实际值
echo 'export DIDI_MCP_KEY="YOUR_KEY"' >> ~/.zshrc
source ~/.zshrc
```

持久化成功后：
- 新开的 shell / 新的 Codex 会话会自动读取 `DIDI_MCP_KEY`
- 当前已存在的会话若未重新加载 shell 配置，可能仍读不到新值，需要 `source ~/.zshrc` 或重开会话

⚠️ Key 未配置**或** MCP 返回鉴权失败（`error.code: -32002`）时，执行以下两步：

1. 直接在回复中给出二维码图片或官网链接。
2. 输出文字：

> 您还没有配置 DIDI_MCP_KEY 或 Key 已失效，请访问 [滴滴MCP平台](https://mcp.didichuxing.com/claw) 获取 MCP KEY，然后配置环境变量或写入 shell 启动文件。
