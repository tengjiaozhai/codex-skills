...
> `<skill-dir>` 代表 didi-ride-skill 技能的安装根目录（即 `SKILL.md` 所在目录）。
...
2. **当前 shell 未注入环境变量**：执行 `printenv DIDI_MCP_KEY` 验证；若为空，先在当前 shell `export DIDI_MCP_KEY=<key>`，或把该变量写入 `~/.zshrc` 后重新加载。
3. **Shell 配置尚未重新加载**：如果刚写入 `~/.zshrc`，但当前 shell 仍报错，执行 `source ~/.zshrc` 或新开一个 shell 后重试。
4. **Key 本身确实无效**：若以上均排除，再按 `### 3.9 MCP KEY 与配置` 流程重新配置。
