...
## Phase 6：预约出行（Codex 自动化）

说明：MCP API 本身是实时发单。只有当用户**明确要求**“15 分钟后打车”“明天 9 点叫车”这类定时需求时，才使用 Codex 自动化能力创建一次性任务。

执行规则：

1. 自动化任务中必须写入完整起终点（带城市前缀）和车型，不能依赖当前对话历史。
2. 起点或终点缺失时，先用 `assets/PREFERENCE.md` 推断并向用户确认；推断不了就继续询问，不要自行猜测。
3. 到点后的执行流程不变：地址解析 → 价格预估 → 创建订单。
4. 如果当前环境不支持自动化，就明确告诉用户本次无法定时执行，请用户到时再来触发。
