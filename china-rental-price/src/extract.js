...
export async function collectReferencePricing(page, options = {}) {
  const selectors = [
    "[class*='price']",
    "[class*='Price']",
    "[class*='amount']",
  ...
  const selectorTexts = await page.locator(selector).evaluateAll((nodes) => {
    return nodes
      .map((node) => ({
  ...
  const textSignals = dedupePriceSignals([
    ...selectorTexts.flatMap((item) => extractPriceSignalsFromText(item.text)),
    ...extractPriceSignalsFromText(bodyText)
  ]);

  const summary = summarizeSignals(textSignals);
  return {
    ...summary,
    signals: textSignals,
    bookingUrl: page.url()
