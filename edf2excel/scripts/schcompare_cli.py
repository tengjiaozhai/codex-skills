...
用法：
    python -m scripts.schcompare_cli edif-export input.edf -o full_edif.xlsx [--json full_edif.json]
...
def _run_edif_export(input_path: Path, output: Path, json_output: Path | None) -> None:
    """导出单个 EDIF 文件的全量语义表。"""
    from .edif_full_extract import extract_full_edif
    from .edif_full_export import export_full_edif_json, export_full_edif_xlsx

    print(f"正在导出全量 EDIF 信息: {input_path}")
    tables = extract_full_edif(input_path)
    meta = {
        "inputPath": str(input_path),
        "mode": "edif-export",
    }
    export_full_edif_xlsx(output, tables, meta)
    print(f"Full EDIF Excel exported: {output}")
    if json_output is not None:
        export_full_edif_json(json_output, tables, meta)
        print(f"Full EDIF JSON exported: {json_output}")
...
示例：
  %(prog)s check-env
  %(prog)s edif-export input.edf -o full_edif.xlsx --json full_edif.json
...
""",
    ...
    export_sp = sub.add_parser("edif-export", help="导出单个 EDIF 文件的全量语义表")
    export_sp.add_argument("input_path", type=Path, help="输入 EDIF 文件")
    export_sp.add_argument(
        "-o", "--output", type=Path, required=True, help="输出 Excel 路径 (.xlsx)"
    )
    export_sp.add_argument("--json", type=Path, default=None, help="可选 JSON 输出路径")
    ...
    if args.command == "edif-export":
        if not args.input_path.exists():
            print(f"错误: 找不到输入文件 {args.input_path}")
            sys.exit(1)
        _run_edif_export(args.input_path, args.output, args.json)
        return
